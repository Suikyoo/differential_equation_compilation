...
class Orbit:
    def __init__(self, center, radius, color=(255, 255, 255), width=1):
        ...
        self.wheel_angle = 0
        self.prev_wheel_angle = 0

        #angle relative to the mouse at first click
        self.virtual_angle = 0
        self.spring_const = 0.001
        self.friction_constant = 0.003
        self.point_mass = 10
    ...
    def physics(self, dt):
        ...
        future_angle = self.wheel_angle + (angular_acc * dt) / self.friction_constant + (self.wheel_angle - self.prev_wheel_angle - (angular_acc * dt) / self.friction_constant) / (math.pow(math.e, self.friction_constant * dt))

        self.prev_wheel_angle = self.wheel_angle
        self.wheel_angle = future_angle
        self.virtual_angle += (self.wheel_angle - self.prev_wheel_angle)
        #small de implementation 
        self.orbit_angle = -self.wheel_angle * (core_functs.divide(self.wheel_radius, self.orbit_radius + self.wheel_radius))
    ...
    def draw(self, surf):
        ...
        if self.mouse_pressed:
            pygame.draw.line(surf, (255, 180, 0), [center[i] + (self.spread + self.wheel_radius) * core_functs.trig_functs[i](self.virtual_angle) for i in range(2)], [round(p/2) for p in pygame.mouse.get_pos()], 3)
        #pygame.draw.circle(surf, self.color, center, self.wheel_radius, self.width)
        pygame.draw.circle(surf, (235, 232, 157), [center[i] + core_functs.trig_functs[i](self.wheel_angle) * self.wheel_radius for i in range(2)], 3)
        """
