...
def outline(surf, coords, mask, color=(255, 255, 255), line_width=1):
    points = mask.outline()
    if len(points) >= 2:
...
#not tested
def line_line_collide(line_1, line_2):
    l = [line_1, line_2]
    dist_list = []

    for i in range(2):
        line_diff = [lines[1][j] - lines[0][j] for j in range(2)]
        k = (i + 1) % 2
        dist = ((l[i][1][0] - l[i][0][0]) * (l[k][0][1] - l[i][0][1]) - (l[i][1][1] - l[i][0][1]) * (l[k][0][0] - l[i][0][0])) / ((l[i][1][1] - l[i][0][1]) * (l[k][1][0] - l[k][0][0]) - (l[i][1][0] - l[i][0][0]) * (l[k][1][1] - l[k][0][1]))
        if 0 <= n <= 1:
            pass
        else: 
            return False
...
#not tested
def rect_line_collide(rect, line):
    rect_lines = []
    start_loc = rect.topleft
    rect_arrangement = ("topleft", "bottomleft", "bottomright", "topright")
    point_index = 1
    for i in range(4):
        end_loc = getattr(rect, rect_arrangement[point_index])
        rect_lines.append((start_loc, end_loc))
        start_loc = end_loc
        point_index = (point_index + 1) % 4

    for j in rect_lines:
        if line_line_collide(j, line):
            return True
